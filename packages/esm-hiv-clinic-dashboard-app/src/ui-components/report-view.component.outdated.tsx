...
describe('<ReportView/>', () => {
  ...
  it('should render report view correctly', () => {
    expect(screen.getByText(/Location Test/i).textContent).toEqual('Location Test');
    expect(screen.getByText(/OVC in HIV/i).textContent).toEqual('OVC in Hiv');
    expect(screen.getByText(/Active in Care and Eligible for OVC/i).textContent).toEqual(
      'Active in Care and Eligible for OVC',
    );
    expect(screen.getByText('-')).toBeInTheDocument();
  });

  it('should redirect to patient list on indicator clicked', () => {
    const activeInCareIndicator = screen.getByText(/Active in Care and Eligible for OVC/i);
